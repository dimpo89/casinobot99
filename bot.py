...
async def callback_handler(self, callback: types.CallbackQuery, state: FSMContext):
    ...
    async def handle_slot_play(self, callback: types.CallbackQuery, state: FSMContext, game_type: str, bet: int, lang: str, **kwargs):
        ...
        if game_type in ["doghouse", "sugarrush"]:
            ...
            await self.db.update_jackpot(int(bet * JACKPOT_PERCENT))
            user_cur = await self.db.get_user(user_id)
            user_pf = user_cur.get('pf_enabled', 1) if user_cur else 1
    ...
    # ---------- Методы для админки (редактор слотов) ----------
    ...
    # ---------- Методы для отображения настроек, бонусов и т.п. ----------
    ...
    async def claim_daily(self, callback: types.CallbackQuery, lang: str):
        await callback.answer("❌ Ежедневный бонус отключён", show_alert=True)
...
async def handle_custom_bet(message: types.Message, state: FSMContext):
    ...
    # Создаём фиктивный callback для вызова handle_slot_play
    ...
    await casino.handle_slot_play(fake_callback, state, game_type, bet, user.get('language','ru'), **kwargs)
...
async def handle_bonus_code(message: types.Message, state: FSMContext):
    ...
    if data.get('admin_action') == "create_bonus":
        ...
        if res is None:
            ...
            await message.answer(f"✅ Бонус активирован! Получено {res['amount']} ⭐")
        await state.clear()
...
async def handle_bonus_code_expiry(message: types.Message, state: FSMContext):
    try:
        ...
        # Здесь нужно выбрать тип кода, пока по умолчанию deposit
        await db.create_bonus_code(code, amount, 'deposit', 35, uses, expiry, message.from_user.id)
        await state.clear()
...
@dp.message(BetStates.waiting_for_tilt_factor)
async def handle_tilt_factor(message: types.Message, state: FSMContext):
    try:
        factor = float(message.text)
        if factor < 0.1 or factor > 10.0:
            await message.answer("❌ Множитель должен быть от 0.1 до 10.0")
            return
        await state.update_data(tilt_factor=factor)
        await state.set_state(BetStates.waiting_for_tilt_hours)
        await message.answer("⏱ На сколько часов установить (0 – навсегда)?")
    except:
        await message.answer("❌ Введите число")

@dp.message(BetStates.waiting_for_tilt_hours)
async def handle_tilt_hours(message: types.Message, state: FSMContext):
    try:
        hours = int(message.text)
        data = await state.get_data()
        game = data['tilt_game']
        factor = data['tilt_factor']
        await db.set_tilt(game, factor, hours)
        await message.answer(f"✅ Для игры {game} установлен множитель {factor} на {hours if hours>0 else 'бессрочно'} часов")
    except:
        await message.answer("❌ Введите число")
    await state.clear()
